...
export class ExistsConstraint implements ValidatorConstraintInterface {
	...
	defaultMessage(validationArguments: ValidationArguments): string {
		return `${validationArguments.property} of ${validationArguments.targetName} with value ${validationArguments.value} not found`
	}
}

interface ExistsParams<T> {
	entity?: Class<T>
	key?: keyof InstanceType<Class<T>>
}

export function Exists<T>(params?: ExistsParams<T> & ValidationOptions) {
